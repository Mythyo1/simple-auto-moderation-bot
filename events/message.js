...
message.delete();
message.channel.send(` ${inviteblit.guild.name}(\`${inviteblit.guild.id}\` ОТ ${message.author}(\`${message.author.id}\``);
};
};
let prefixe111 = [`${process.env.prefix}`,`<@${bot.user.id}>`, `<@!${bot.user.id}>`]
